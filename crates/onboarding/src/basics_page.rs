...
const MISTRAL_VIBE_AGENT_ID: &str = "mistral-vibe";

pub(crate) const FEATURED_AGENT_IDS: &[&str] = &[
    MISTRAL_VIBE_AGENT_ID,
    "claude-acp",
    "codex-acp",
    "github-copilot-cli",
    "cursor",
...
fn render_mistral_vibe_button() -> impl IntoElement {
    AgentSetupButton::new("mistral-vibe-onboarding")
        .icon(
            Icon::new(IconName::AiMistral)
                .size(IconSize::XSmall)
                .color(Color::Muted),
        )
        .name("Mistral Vibe")
...
fn render_ai_section(cx: &mut App) -> impl IntoElement {
    ...
    let grid = FEATURED_AGENT_IDS.iter().skip(1).fold(
        div()
            .w_full()
            .mt_1p5()
            .grid()
            ...
            .gap_2()
            .child(render_mistral_vibe_button()),
        |grid, agent_id| {
...
mod tests {
    ...
    #[test]
    fn features_mistral_vibe_first() {
        assert_eq!(FEATURED_AGENT_IDS.first(), Some(&"mistral-vibe"));
    }
}
